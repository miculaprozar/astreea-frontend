...
import { formatDuration, chargeDate, chargeLastUsed } from "../../helpers/formatFunctions";
...
const HomeChargerCard = ({ charger, onClick }) => {
    ...
    useEffect(() => {
        ...
        if(!global['ChargerStateChanged=' + chargerState.serialNumberCon]){
            console.log("Subscribed to:" + 'ChargerStateChanged=' + chargerState.serialNumberCon);
            
            global['ChargerStateChanged=' + chargerState.serialNumberCon] = true;
            connection.on('ChargerStateChanged=' + chargerState.serialNumberCon, (newState) => {
        ...
        if(!global['ChargingChanged=' + chargerState.serialNumberCon]){
            ...
            connection.on('ChargingChanged=' + chargerState.serialNumberCon, (chargingChange) => {        
    ...
    const StartStopCharging = async () => {
        ...
        if(startStopOngoing == false){
            ...
            commandTimeoutId = setTimeout(() => {setStartStopOngoing(false);}, 30000);
            ...
            console.log("Cannot perform Start/Stop operation while another operation is already in progress.");
        }
                            ...
                            <View style={{ marginTop: "auto" }}>
                                ...
                                >
                                    {chargerState.lastChargingSession
                                        ? (chargerState.lastChargingSession.charged/10000).toFixed(2) + "kWh"
                                        : "--"}
                                </Text>
                            </View>
                            <View style={{ marginTop: "auto" }}>
                                ...
                                >
                                    {chargerState.lastChargingSession
                                        ? (formatDuration(chargerState.lastChargingSession.chargedTimeInSec))
                                        : "--"}
                                </Text>
                            </View>
                            <View style={{ marginTop: "auto" }}>
                    ...
                    {(
                        ...
                        >
                            <ChargerButton
                                ...
                                isDisabled={(chargerState.state === "Occupied" || startStopOngoing) ? true : false}
                                onPressAction={() =>
